    ...
    <div className="guide-fromtrack">
      ...
      {busy && (
        <div style={{ margin: "4px 0 8px" }}>
          <div className="stt-prog" />
          <div className="settings-desc-small" style={{ marginTop: 4 }}>{tr("stt.working")}{(state.ttsBusy?.pct ?? 0) > 0 ? ` · ${state.ttsBusy?.pct}%` : ""}</div>
        </div>
      )}
        ...
        {importResult && (
          <div className="import-result">
            {importResult.total === 0 ? (
              // Nothing was READ from the file — almost always a printed/graphic PDF with no text
              // layer (AbleJam's own print PDF is like this). Guide to the re-importable path.
              <div className="ir-empty">{tr("import.result.empty")}</div>
            ) : (
              <>
...
function PrintView({ state, onClose }: { state: AppState; onClose: () => void }) {
  ...
  // A PDF generated by AbleJam with a REAL text layer (jsPDF), so it re-imports cleanly — unlike a
  // browser "Print → Save as PDF", which can outline the fonts (no text) and yields 0 matches. One
  // entry per line (no wrapping) so pdf-parse reads one title-line per song. jsPDF is loaded on demand.
  const exportPdf = async () => {
    ...
    const M = 42;
    const title = band.trim() || tr("print.defaultTitle");
    let y = 60;
    doc.setFont("helvetica", "bold"); doc.setFontSize(20);
    doc.text(title, M, y);
    doc.setFont("helvetica", "normal"); doc.setFontSize(10); doc.setTextColor(130);
    doc.text(`${items.length} · ${today}`, PW - M, y, { align: "right" }); // digits-only line → ignored on re-import
    if (evt.trim()) { y += 16; doc.text(evt, M, y); }
    doc.setTextColor(0); doc.setFontSize(12);
    y += 28;
    for (const it of items) {
      if (y > PH - 44) { doc.addPage(); y = 56; }
      const body = it.songs.map((s) => s.title).join(" / ")
        + (it.songs.length === 1 && it.songs[0]!.key ? ` (${it.songs[0]!.key})` : "");
      doc.text(`${it.num}. ${body}`, M, y); // "N. " (with dot) is stripped by the import parser
      y += 22;
    }
    doc.save(`${title.replace(/[^\w.-]+/g, "_") || "setlist"}.pdf`);
  };
  return (
